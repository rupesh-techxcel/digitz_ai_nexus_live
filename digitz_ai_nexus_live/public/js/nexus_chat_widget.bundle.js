...
(function (global) {
            ...
            <div id="ncw-panel" style="display:none;">
                ...
                <div id="ncw-wa-panel" style="display:none;">
                    ...
                    <div id="ncw-wa-otp-step" style="display:none;">
                        ...
                        <div class="ncw-wa-row">
                            <input id="ncw-wa-otp-input" type="text" placeholder="······" maxlength="6" inputmode="numeric" autocomplete="one-time-code"/>
    ...
    function bind_ui_events() {
        ...
        async function _wa_send_otp() {
            ...
            if (!phone) { _wa_phone_err('Please enter your WhatsApp number.'); return; }
            ...
            try {
                ...
                if (d.status === 'sent') {
                    ...
                    if (lbl && d.phone) lbl.textContent = 'Enter the 6-digit code sent to WhatsApp ' + d.phone + ':';
        ...
        async function _wa_verify_otp() {
            ...
            if (!otp) { _wa_otp_err('Please enter the verification code.'); return; }
            ...
            try {
                ...
                // Success handled via realtime event 'whatsapp_registered'
            } catch (err) {
    ...
    function _init_tooltip() {
        ...
        // Don't show on desk
        if (is_desk()) { tip.style.display = 'none'; return; }
        ...
        if (dismiss_btn) dismiss_btn.addEventListener('click', function (e) {
            e.stopPropagation();
            _dismiss_tooltip();
        });
        if (cta_btn) cta_btn.addEventListener('click', function () {
            _dismiss_tooltip();
            open_panel();
        });

        // Reveal after a short delay, auto-dismiss after 10 s
        setTimeout(function () {
            ...
            setTimeout(_dismiss_tooltip, 10000);
        }, 3000);
    }
    ...
    function bind_realtime() {
        frappe.realtime.on('nexus_chat_response', function (data) {
            ...
            // WhatsApp registration confirmed — switch panel to done state
    ...
    function render_identity_verification_prompt() {
        ...
        document.getElementById('ncw-verify-email').addEventListener('keydown', function (e) {
            if (e.key === 'Enter') { e.preventDefault(); submit_email_for_verification(prompt); }
        });
    ...
    async function submit_otp_verification(prompt_el, challenge_token) {
        ...
        try {
            ...
            if (data.status === 'verified') {
                ...
                prompt_el.innerHTML =
                    '<div class="ncw-verify-msg ncw-verify-success">✓ Identity verified as <strong>' +
                    escape_html(data.identity_type || 'Verified') +
                    '</strong>. Your next message will use your verified access.</div>';
                scroll_bottom();
            }
        } catch (err) {
            btn.disabled    = false;
